...
THREE.GPUParticleSystem = function( options ) {
	...
	var GPUParticleShader = {
		...
		vertexShader: [
			...
			'	vec3 noiseOld = texture2D( tOldNoise, vec2( newPosition.x * 0.015 + ( uTime * 0.05 ), newPosition.y * 0.02 + ( uTime * 0.015 ) ) ).rgb;',
			'   vec3 noiseNew = texture2D( tNoise, vec2( newPosition.x * 0.015 + ( uTime * 0.05 ), newPosition.y * 0.02 + ( uTime * 0.015 ) ) ).rgb;',
			'   vec3 noiseMix = mix(noiseOld, noiseNew, mixVal);',
			'	vec3 noiseVel = ( noiseMix.rgb - 0.5 ) * 30.0;',
	...
	this.particleShaderMat = new THREE.ShaderMaterial( {
		...
		uniforms: {
			...
			'tOldNoise': {
				value: new THREE.DataTexture([0,0,0,1], 1, 1, THREE.RGBAFormat)
			},
			'tNoise': {
